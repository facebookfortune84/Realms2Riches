...
**NEW DIRECTIVE:** We are pivoting to become an "Autonomous Software Engineer" platform (similar to Famous.ai but sovereign). The system must be capable of generating, deploying, and monetizing full-stack applications from natural language prompts.
...
6.  **Famous Mode (App Generation):** 
    *   **Sovereignty:** All generated apps must be fully self-contained (Dockerized) with no platform lock-in.
    *   **Monetization First:** Every generated app must have Stripe checkout pre-configured using `.env.prod` keys.
    *   **Scale:** The builder agent must handle complex prompts and output production-ready code structures.
...
1.  **Initiate:** Launch streams via `scripts/yolo_mode_monetization.py` or generate apps via `scripts/famous_mode.py`.
